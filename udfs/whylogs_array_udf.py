...
class handler:
    def end_partition(self, df):
        """
        Args:
            df: Apparently, when the sql type is ARRAY, the df is a single series and each series has a list of values.
        """
        return pd.DataFrame({"profile_view": [str(df[0])], 'total_processed': [len(df)], 'random_number': [-1]})
        # id_col = df[0]
        # name_col = df[1]
        # department_col = df[2]
        # age_col = df[3]
